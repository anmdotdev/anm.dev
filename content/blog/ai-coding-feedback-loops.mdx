...
summary: "Most bad AI code is not a model problem. It is a feedback-loop problem: if an agent cannot see, test, and correct its work, it codes blind."
...
The model writes some code.
Then I open the browser, click around, read the console, notice the failed request, explain the failure back to the model, and repeat.
...
Yes, context matters.
Yes, better prompts help.
Yes, it is useful to explain the intent precisely.

But the real unlock is giving the agent a way to validate its own work.

<Callout type="note">
The missing context is often not more prose. It is executable feedback. Intent + Validation = Loop
</Callout>
...
So the model produces a draft.
The human becomes the test harness.
The human becomes the browser.
The human becomes the console.
The human becomes the network inspector.
...
Intent tells the agent what to build. Validation tells it how to know it is done.
</Callout>
...
In practice, that means tools like [Playwright Skills](https://github.com/lackeyjb/playwright-skill) and [Chrome DevTools Skills](https://github.com/github/awesome-copilot/blob/main/skills/chrome-devtools/SKILL.md).
